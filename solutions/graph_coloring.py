...
def color_graph(graph, colors):
    ...
    # For each node see if neighbours have colour
    ...
    for node in graph:
        ...
        for neighbour in node.neighbors:
            if neighbour == node:
                raise Exception
            neighbours.add(neighbour.color)

        legal_colours = colors - neighbours
        prefered = colours_used & legal_colours
        if prefered:
            node.color = prefered.pop()
        else:
            node.color = legal_colours.pop()
